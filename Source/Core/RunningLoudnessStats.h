...
//
// Computes:
// - Integrated loudness (running, EBU-style gated) from 400ms blocks @ 10 Hz
// - LRA (running, Tech 3342 style) from short-term values @ 1 Hz (10th/95th pct)
//
// Notes:
// - This is "running" and updated as frames arrive.
// - On DAW discontinuities (seek/loop), call reset().
// - This class does no allocations in pushFrameEnergy().
//==============================================================================
...
public:
    ...
    // Call once per 60 Hz loudness frame with BS.1770 K-weighted mean-square energy.
...
private:
    //==============================================================================
    // Constants / conversions (BS.1770)
    //==============================================================================

    static float energyToLufs (float meanSquare) noexcept;
    static float lufsToEnergy (float lufs) noexcept;
    ...
    // Sliding window for 400ms blocks
    ...
    // store count and sum of block mean-square energy per loudness bin
    ...
    //==============================================================================
    // LRA (1 Hz, short-term from 3s window)
    //==============================================================================
    ...
    // Histogram of short-term loudness values (absolute gated only)
    std::array<int, histBins> stCount {};
    int stTotalCount = 0;

    //==============================================================================
    // Published results (atomics for safe UI reads)
    //==============================================================================
